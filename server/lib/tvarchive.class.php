    ...
    public function createLink(){
        ...
        if ($storage['dvr_type'] == 'flussonic_dvr' ){
            ...
            if (preg_match("/:\/\/([^\/]*)\/.*\.m3u8/", $channel['mc_cmd'], $match)){
                ...
                $res['cmd'] .= '&'.Itv::getWowzaSecureToken($res['cmd'], Config::getSafe('wowza_securetoken_vod_endtime', 0));
            ...
            if (preg_match("/:\/\/([^\/]*)\/.*\.(mpd|m3u8)/", $channel['mc_cmd'], $match)){
                ...
                $res['cmd'] = preg_replace('/:\/\/([^\/]*)/', '://'.$storage['storage_ip'], $channel['mc_cmd']);
                $res['cmd'] = preg_replace('/\.' . $match[2] . '.*/',
                        '_range-' . $start_timestamp . '-' . ($stop_timestamp - $start_timestamp) . '.' . $match[2],
                        $res['cmd'])
                    . '?token='.$this->createTemporaryToken("1");

                $res['cmd'] .= '&'. preg_match('/https?\:\/\//i', $res['cmd']) ? Itv::getNimbleHttpAuthToken($res['cmd']) : Itv::getNimbleRtspAuthToken($res['cmd']);
    ...
    public function getUrlByProgramId($program_id, $disable_overlap = false, $program = array()){
        ...
        if ($storage['dvr_type'] == 'flussonic_dvr'){
            ...
            if (preg_match("/:\/\/([^\/]*)\/.*\.m3u8/", $channel['mc_cmd'], $match)){
                ...
                $url .= '&'.Itv::getWowzaSecureToken($url, Config::getSafe('wowza_securetoken_vod_endtime', 0));
            ...
            if (preg_match("/:\/\/([^\/]*)\/.*\.(mpd|m3u8)/", $channel['mc_cmd'], $match)){
                ...
                $url .= '&'. preg_match('/https?\:\/\//i', $url) ? Itv::getNimbleHttpAuthToken($url) : Itv::getNimbleRtspAuthToken($url);
    ...
    public function getLinkForChannel(){
        ...
        if ($channel['tv_archive_type'] == 'flussonic_dvr'){
            ...
            if (preg_match("/:\/\/([^\/]*)\/.*\.(mpd|m3u8)/", $channel['mc_cmd'], $match)){
                ...
                $res['cmd'] = preg_replace('/dvr.*\.' . $match[2] . '.*/',
                        'dvr_range-' .  gmmktime(gmdate('H'), 0, 0) . '-3600.' . $match[2] , // todo: in current hour without duration?
                        $url)
                    . '?token='.$this->createTemporaryToken("1");
                $res['cmd'] .= '&'. preg_match('/https?\:\/\//i', $res['cmd']) ? Itv::getNimbleHttpAuthToken($res['cmd']) : Itv::getNimbleRtspAuthToken($res['cmd']);
